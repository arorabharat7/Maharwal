...
<div class="absolute bottom-6 left-[50%] translate-x-[-50%] flex items-center md:gap-2.5 gap-1.5">
  ...
  <a href="#" class="text-white md:text-lg text-base font-medium">Blogs</a>
...
if ( $query->have_posts() ) {
    ...
    <div class="grid grid-cols-12 gap-x-6 xl:gap-y-16 md:gap-y-10 gap-y-8 lg:pb-100 md:pb-60 pb-10">
      ...
      <div class="md:col-span-4 sm:col-span-6 col-span-12 relative">
        <span
          class="lg:text-xl md:text-base text-sm text-white bg-primary rounded-bl-xl rounded-tr-xl py-1 px-2 absolute right-0 top-0">
    ...
    <span class="xl:text-sm text-xs text-primary font-medium uppercase inline-block mr-1"><?php echo esc_html( $category->name ); ?></span>
<?php } ?>
          <span
            class="xl:text-sm text-xs text-primary font-medium uppercase inline-block lg:ml-0 ml-3 relative after:content-[''] after:absolute after:-left-3 after:top-[50%] after:translate-y-[-50%] after:bg-secondary after:h-2 after:w-2 after:rounded-full">
        ...
        <a href="<?php the_permalink(); ?>" class="btn text-sm font-semibold text-light-black text-center inline-block uppercase">read more <i
